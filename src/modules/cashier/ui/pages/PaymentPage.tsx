...
import { useCashierContextQuery, useCashierPaymentMutation, useCashierPaymentOrderQuery } from 'modules/cashier/application';
...
export function PaymentPage() {
  ...
  const serviceFeePercent = Number(orderQuery.data?.serviceFeePercent ?? (orderQuery.data?.channel === 'hall' ? 10 : 0));
      ...
      header={
        <Stack
          ...
          <Stack direction="row" spacing={{ xs: 1, md: 1.5 }} sx={{ justifyContent: { xs: 'flex-end', md: 'flex-start' } }}>
            {!isMobile ? <PosIconAction icon="solar:refresh-bold-duotone" onClick={() => window.location.reload()} /> : null}
            <PosIconAction
              icon="solar:settings-bold-duotone"
              onClick={(event) => setSettingsAnchor(event.currentTarget)}
            />
            {!isMobile ? <PosIconAction icon="solar:lock-password-bold-duotone" onClick={() => navigate('/lock-screen')} /> : null}
          </Stack>
        </Stack>
      }
    >
          ...
          <Stack spacing={2}>
            <Stack direction="row" spacing={1.5} alignItems="center">
              <Box
                ...
                {orderQuery.data?.channel === 'takeaway' ? 'TG' : (orderQuery.data?.tableName?.match(/\d+/)?.[0] ?? '0')}
              </Box>
              <Stack spacing={0.25}>
      ...
      <Dialog open={Boolean(receiptData)} onClose={() => setReceiptData(null)} maxWidth="xs" fullWidth fullScreen={isMobile}>
