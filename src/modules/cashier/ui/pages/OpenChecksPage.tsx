...
function OpenChecksDetail({
  ...
  latestSucceededPayment: CashierOrder['payments'][number] | undefined;
  locale: string;
  onPay: () => void;
  onRefund: () => void;
  onReprint: () => void;
...
export function OpenChecksPage() {
  ...
  const latestSucceededPayment = useMemo(
    () => [...(selectedOrder?.payments ?? [])].filter((payment) => payment.status === 'succeeded').at(-1),
    [selectedOrder?.payments],
  );
  const latestReceipt = useMemo(() => [...(selectedOrder?.receipts ?? [])].at(-1), [selectedOrder?.receipts]);
  const receiptNumber = useMemo(() => {
    const payload = latestReceipt?.payload as Record<string, unknown> | undefined;
    return payload?.receiptNumber ?? payload?.receipt_number ?? latestSucceededPayment?.id ?? copy.receiptUnavailable;
